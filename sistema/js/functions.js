$(document).ready(function(){
    ...
    $('.add_product').click(function(e) {
            ...
            $.ajax({
                ...
                if(response != 'error'){
                    ...
                    $('.bodyModal').html('<form action="" method= "post" name="form_del_product" id="form_del_product" onsubmit="event.preventDefault(); delProduct();">'+									
                                    ...
                                    '<p>¿Estas seguro de querer eliminar el siguiente registo?</p>'+
                                    '<h2 class="nameProducto">'+info.descripcion+'</h2><br>'+
                                    '<input type="hidden" name="producto_id" id="producto_id" value="'+info.codproducto+'" required>'+
                                    ...
                                    '<button href="#" type="submit" class="btn_ok"><i class="fa-solid fa-trash"></i> Eliminar</button>'+
                                    '</form>');
                }
                },
                error: function(error){
        ...
        $('#txt_cant_producto').keyup(function(e){
            ...
            var existencia = parseInt($('#txt_existencia').html()); //se transforma lo que se trae en un entero con el parseInt()
            $('#txt_precio_total').html(precio_total);
        //oculta el boton agregar si el boton es menor a 1
        ...
        if(rows > 0)
        {
            ...
            $.ajax({
                ...
                success: function(response)
                {
                    if(response != 'error')
                    {
                        location.reload();//refrescar la pag
                    }
                },
                error: function(error) {
                }
    ...
    $('#btn_facturar_venta').click(function(e){
                ...
                success: function(response){
                    if(response != 'error'){
                        ...
                        $('.bodyModal').html('<form action="" method= "post" name="form_anular_factura" id="form_anular_factura" onsubmit="event.preventDefault(); anularFactura();">'+									
                                        ...
                                        '<p>¿Estas seguro de querer anular la factura?</p>'+
                                        ...
                                        '<div class="alert alertDelProduct"></div>'+
                                        '<button href="#" type="submit" class="btn_ok"><i class="fa-solid fa-trash"></i> Anular</button>'+
                                        '<a href="#" class="btn_cancel closeModal" onclick="closeModal();"><i class="fas fa-ban"></i> Cerrar</a>'+
        ...
        $('#frmChangePass').submit(function(e){
            ...
            if(passNuevo.length < 6){
                $('.alertChangePass').html('<p>La nueva contraseña debe tener 6 caracteres</p>');
                $('.alertChangePass').slideDown();
                return false;
            }
        ...
        $.ajax({
            ...
            success: function(response)
            {
                ...
                if(info.cod == '00')
                {
                    ...
                    $('.alertFormEmpresa').slideDown();
                    $('#frmEmpresa input').remove('disabled');
                },
                error: function(error){
                }
                ...
                $.ajax({
                    ...
                    success: function(response){
                        if(response == 'error'){
                            ...
                            $('#row_'+noFactura+' .estado').html('<span class="Anulada"></span>');
